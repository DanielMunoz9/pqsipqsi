...
# Download Piper TTS Linux runtime
RUN mkdir -p /opt/piper && \
    ...
    if [ -f /opt/piper/piper/piper_phonemize ]; then chmod +x /opt/piper/piper/piper_phonemize; fi

# Download voices
RUN mkdir -p /opt/piper-voices && \
    wget -q -O /opt/piper-voices/es_ES-carlfm-x_low.onnx \
      "https://huggingface.co/rhasspy/piper-voices/resolve/main/es/es_ES/carlfm/x_low/es_ES-carlfm-x_low.onnx" && \
    wget -q -O /opt/piper-voices/es_ES-carlfm-x_low.onnx.json \
...
# Put Piper runtime where main.go expects it: tts/piper/piper/piper
RUN mkdir -p tts/piper/piper tts/voices && \
  cp -R /opt/piper/piper/. tts/piper/piper/ && \
  chmod +x tts/piper/piper/piper && \
    cp /opt/piper-voices/es_MX-claude-high.onnx tts/voices/ && \
  cp /opt/piper-voices/es_MX-claude-high.onnx.json tts/voices/ && \
  cp /opt/piper-voices/es_ES-carlfm-x_low.onnx tts/voices/ && \
  cp /opt/piper-voices/es_ES-carlfm-x_low.onnx.json tts/voices/
